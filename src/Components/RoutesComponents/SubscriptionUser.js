...
const SubscriptionUser = () => {
  ...
  const { driverSubscriptionData, RecieveData } = useSelector((state) => state.panelSlice);
  ...
  useEffect(() => {
    setDriverList(driverSubscriptionData.filter((item, index) => String(item.planName).toLowerCase().includes(searchTxt) || String(item.person).includes(searchTxt) || String(item.startDate).includes(searchTxt) || String(item.endDate).includes(searchTxt) || String(item.price).includes(searchTxt)  ));
  }, [searchTxt]);
          ...
          <div className="flex justify-around formDiv w-full flex-wrap gap-5">
            ...
            <div>
              ...
              >
                ...
                <option value="inactive" defaultChecked>Inactive</option>
              </select>
            </div>
          </div>
          <button type="submit" className="bg-[#8c7fe6] hover:bg-[#8c89cf] border-2 active:border-black active:text-black text-white font-bold py-2 px-4 rounded mt-4">
            Add
          </button>
        </form>
      </div>
        ...
        <div className={`data mt-2 flex flex-col gap-1 max-h-[520px] overflow-y-auto   `}>
          ...
          {driverList.length > 0 && driverList.map((item, index) => <>
            <div key={index} className="dataList bg-white py-2 rounded-lg flex justify-around">
              ...
              <span>|</span>
              <button className={`w-[120px] rounded-xl text-sm p-1 ${item.status === "Active" ? 'bg-[#7ec067]' : item.status === "Inactive" ? 'bg-[#e98b8b]' : 'bg-[#f8e321]'}`}>{item.status}</button>
              <span>|</span>
              <div className='w-[120px] flex justify-around items-center text-[#8c7fe6]'><FiEye title='View' onClick={() => setDetails({ check: !details.check, id: item.id })} /> <LuHistory title='History' onClick={() => setDrill({ check: !drill.check, id: item.id })} /> <FaRegEdit title='Edit' /> <FaTrash title='Delete' /> </div>
            </div>{drill.check && item.id === drill.id && <DrillDown driverId={item.id} />}</>)}
        </div>
        <Swiper
          slidesPerView={4}
          spaceBetween={1}
...
//   <div>
//     <div className='w-full h-[100px] flex items-center px-10'>
//     <Heading headingText={"Subscription User"}/>
//     </div>
//     <h1>Insurance</h1>
//   </div>
// )